...
void loop(){
  ...
  */
  ...
  // Send every 5000ms
  if(millis() - timer >= 61267 || timer > millis()){
    timer = millis();
   
    modbusData();   // update data modbus convert to float
    sendToServer("REFI_A"); // send to server over HTTP request (POST)
    
  }

  // Send every 5000ms
  if(millis() - timer2 >= 71267 || timer2 > millis()){
    timer2 = millis();

    
 
    modbusData_2();   // update data modbus convert to float
    sendToServer("REFI_B"); // send to server over HTTP request (POST)
  ...
  // Send every 5000ms
  if(millis() - timer3 >= 51267 || timer3 > millis()){
    timer3 = millis();
    ...
    modbusData_3();
    sendToServer("ICE_C"); // send to server over HTTP request (POST)
