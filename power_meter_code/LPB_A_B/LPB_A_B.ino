...
void loop(){
  ...
  */
  ...
  // Send every 5000ms
  if(millis() - timer >= 66660 || timer > millis()){
    timer = millis();
   
    modbusData();   // update data modbus convert to float
    sendToServer("LPB_A"); // send to server over HTTP request (POST)
    
  }

  // Send every 5000ms
  if(millis() - timer2 >= 75600 || timer2 > millis()){
    timer2 = millis();

    
 
    modbusData_2();   // update data modbus convert to float
    sendToServer("LPB_B"); // send to server over HTTP request (POST)
