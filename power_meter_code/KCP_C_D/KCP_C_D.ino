...
void loop(){
  ...
  */
  ...
  // Send every 5000ms
  if(millis() - timer >= 70776 || timer > millis()){
    timer = millis();
   
    modbusData();   // update data modbus convert to float
    sendToServer("KCP_C"); // send to server over HTTP request (POST)
    
  }

  // Send every 5000ms
  if(millis() - timer2 >= 80123 || timer2 > millis()){
    timer2 = millis();

    
 
    modbusData_2();   // update data modbus convert to float
    sendToServer("KCP_D"); // send to server over HTTP request (POST)
