...
void loop(){
  ...
  */
  ...
  // Send every 5000ms
  if(millis() - timer >= 67211 || timer > millis()){
    timer = millis();
   
    modbusData();   // update data modbus convert to float
    sendToServer("PDU4A"); // send to server over HTTP request (POST)
    
  }

  // Send every 5000ms
  if(millis() - timer2 >= 81264 || timer2 > millis()){
    timer2 = millis();

    
 
    modbusData_2();   // update data modbus convert to float
    sendToServer("PDU4B"); // send to server over HTTP request (POST)
